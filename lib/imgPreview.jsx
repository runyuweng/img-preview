...
class ImgPreview extends PureComponent {
  ...
  initPosition = () => {
    ...
    if (imgRect.left > conRect.left && imgRect.width > conRect.width) {
      this.img.style.left = `${parseInt(this.translatePx(this.img.style.left) - (imgRect.left - conRect.left), 10)}px`
    }

    if (imgRect.top > conRect.top) {
      this.img.style.top = `${parseInt(this.translatePx(this.img.style.top) - (imgRect.top - conRect.top), 10)}px`
    }

    if (imgRect.right < conRect.right && imgRect.width > conRect.width) {
      this.img.style.left = `${parseInt(this.translatePx(this.img.style.left) + (conRect.right - imgRect.right), 10)}px`
    }

    if (imgRect.bottom < conRect.bottom) {
      this.img.style.top = `${parseInt(this.translatePx(this.img.style.top) + (conRect.bottom - imgRect.bottom), 10)}px`
    }
  }

  translatePx = value => (value ? parseInt(value.split('px')[0], 10) : 0)
